...
void doing_process(int who) {
    ...
    printf("I'm %s\n", msg);
    if (who == 0)
    printf("MasterPort = %d, Desci = %s\n", master_port, master_desci);
    else 
    printf("SlavePort = %d, Desci = %s\n", slave_port, slave_desci);
    return ;
}

void exe_master() {
    int pnum = 1;
    DBG(BLUE"The %dth Process act as a Master!\n"NONE, pnum);
    while (1) {
...
void exe_slave() {
    ...
    while (1) {
        if (share_memory->type == 0) {
            pthread_mutex_lock(&share_memory->mutex);
            DBG(BLUE"Slave Process Waiting for Sigal..\n"NONE);
            pthread_cond_wait(&share_memory->cond, &share_memory->mutex);
            pthread_mutex_unlock(&share_memory->mutex);
            while (1) {
                if (share_memory->type == 0) {
...
int main() {
    ...
    for (int i = 0; i < MAX_THREAD; ++i) {
        printf("%s %s %d\n", arr[i].sh, arr[i].conf, arr[i].time);
    }
    */
    ...
    if (pid == 0) {
        ...
        int pnum = 0;
        DBG(RED"The %dth Process act as a Controller!\n"NONE, pnum);
        while(1) {
            int cmd;
